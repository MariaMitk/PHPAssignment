...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {// Έλεγχος αν η φόρμα υποβλήθηκε
    if (isset($_POST['name']) && isset($_POST['language'])&& isset($_POST['ocupation'])) {// Έλεγχος αν το πεδίο name και language υπάρχει στη φόρμα
        $language = $_POST['language']; // Λήψη επιλογής γλώσσας από τη φόρμα
        $name = $_POST['name']; // Λήψη ονόματος από τη φόρμα
        $ocupation = $_POST['ocupation']; //Λήψη επαγγέλματος
...
<html>
    ...
    <h2>Εγγραφή χρήστη</h2>
        ...
        <div class="form-group">
            <label for="ocupation">Επάγγελμα:</label>
        ...
        function validateForm() { // Ορισμός συνάρτησης για την επικύρωση της φόρμας
            ...
            if (ocupation.length < 3) { // Έλεγχος αν η ιδιότητα έχει λιγότερους από 5 χαρακτήρες
                alert("Το πεδίο επάγγελμα πρέπει να περιέχει τουλάχιστον 3 χαρακτήρες."); // Εμφάνιση μηνύματος σφάλματος
                return false; // Ακύρωση υποβολής της φόρμας
            }
